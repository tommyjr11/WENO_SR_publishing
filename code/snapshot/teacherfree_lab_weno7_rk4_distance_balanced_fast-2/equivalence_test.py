...
def main() -> None:
    ...
    original_model = W.ReflectionSymmetricBadnessMLP(seed=91).to(device)
    with torch.no_grad():
    ...
    fast_model = W.ReflectionSymmetricBadnessMLP(seed=0).to(device)
    ...
    tolerance = 2.0e-11 if device.type == "cpu" else 2.0e-10
    if state_target_error > tolerance:
        raise AssertionError(
            f"vectorized cell-average targets differ by {state_target_error}"
        )
    if point_target_error > tolerance:
        raise AssertionError(
            f"vectorized point targets differ by {point_target_error}"
        )
    if loss_relative_error > tolerance:
        raise AssertionError(
            f"total loss relative error={loss_relative_error}"
        )
    if gradient_relative_error > 5.0e-9:
        raise AssertionError(
