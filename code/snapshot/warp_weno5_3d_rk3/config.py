...
class ShockBubbleConfig:
    """Configuration matching the active C++ Ma=3 shock--bubble run."""
    ...
    def validate(self) -> None:
        if min(self.nx, self.ny, self.nz) < 5:
            raise ValueError("WENO5 requires at least five cells per direction")
        if self.ghost != 3:
            raise ValueError("this exact port requires three ghost cells")
        if self.gamma != 1.4:
            raise ValueError("the C++ reference hard-codes gamma=1.4")
        if self.cfl <= 0.0:
