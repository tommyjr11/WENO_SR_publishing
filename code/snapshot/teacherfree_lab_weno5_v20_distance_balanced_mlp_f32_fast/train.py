#!/usr/bin/env python3
"""Fast, numerically equivalent WENO5 V20 mixed-FP32 training."""
...
RECIPE = (
    "weno5_v20_single_cfl_equal_probability_distance_balanced_"
    "mlp_float32_state_float64_fast_execution_v1"
)
PROFILE_NAMES = profiles_module.PROFILE_NAMES


def append_csv(
    path: Path, row: dict[str, object], fields: list[str]
) -> None:
...
def resolve_compile_mode(
    ...
    if requested != "auto":
        return requested
    return "jit" if device.type == "cuda" else "none"
...
def signature(args: argparse.Namespace) -> dict[str, object]:
    return {
        ...
        "loss_execution": (
            "precomputed_targets_chunked_checkpoint_edge_reuse_v1"
        ),
    }
...
def save_model(path: Path, model, step: int, args: argparse.Namespace) -> None:
    W.save_checkpoint(
        ...
        model,
        {
            ...
            "controlled_ablation_source_recipe": (
                "weno5_v20_single_cfl_equal_probability_distance_balanced"
            ),
        },
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    model = ReflectionSymmetricBadnessMLPFloat32(seed=args.seed).to(device)
    parameter_dtypes = {parameter.dtype for parameter in model.parameters()}
    if parameter_dtypes != {torch.float32}:
        raise RuntimeError(
            f"MLP parameters must all be float32, got {parameter_dtypes}"
        )
    ...
    print(
        f"weno5_v20_start initialization={initialization_label} "
        f"steps={args.steps} "
        ...
        "precision=mlp_float32_state_float64 tf32=disabled",
        flush=True,
    )
    ...
    for step in range(start_step, run_end + 1):
        ...
        profiles = profiles_module.make_profiles(
            batch,
            args.grid,
            device,
            gen,
        ...
        if args.eval_interval > 0 and (
            ...
            with torch.no_grad():
                ratios = []
                for name, profile in probes.items():
                    error = A.final_state_error_signed(
                        model,
                        profile,
                        args.eval_grid,
                        args.eval_steps * args.primary_cfl,
