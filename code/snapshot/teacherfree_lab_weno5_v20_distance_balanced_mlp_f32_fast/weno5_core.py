...
class SharedBadnessMLP(torch.nn.Module):
    ...
    def __init__(self, seed: int = 41) -> None:
        ...
        self.w1 = torch.nn.Parameter(
            torch.randn((MLP_INPUTS, MLP_HIDDEN1), generator=gen, dtype=torch.float64)
            * np.sqrt(1.0 / MLP_INPUTS)
        )
        self.b1 = torch.nn.Parameter(torch.zeros((MLP_HIDDEN1,), dtype=torch.float64))
        self.w2 = torch.nn.Parameter(
            torch.randn((MLP_HIDDEN1, MLP_HIDDEN2), generator=gen, dtype=torch.float64)
            * np.sqrt(1.0 / MLP_HIDDEN1)
        )
        self.b2 = torch.nn.Parameter(torch.zeros((MLP_HIDDEN2,), dtype=torch.float64))
        self.w3 = torch.nn.Parameter(
            torch.randn((MLP_HIDDEN2, MLP_HIDDEN3), generator=gen, dtype=torch.float64)
...
def load_checkpoint(path: Path, device: torch.device | str = "cpu") -> SharedBadnessMLP:
    ...
    with torch.no_grad():
        model.w1.copy_(torch.as_tensor(data["w1"][0], device=device))
        model.b1.copy_(torch.as_tensor(data["b1"][0], device=device))
        model.w2.copy_(torch.as_tensor(data["w2"][0], device=device))
        model.b2.copy_(torch.as_tensor(data["b2"][0], device=device))
        model.w3.copy_(torch.as_tensor(data["w3"][0], device=device))
        model.b3.copy_(torch.as_tensor(data["b3"][0], device=device))
        model.w4.copy_(torch.as_tensor(data["w4"][0], device=device))
        model.b4.copy_(torch.as_tensor(data["b4"][0], device=device))
    return model
