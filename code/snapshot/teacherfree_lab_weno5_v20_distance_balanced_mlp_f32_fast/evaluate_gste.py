...
import gste_advection as adv5
from gste import cell_averages, gste_point, integrate_ssprk3
...
def plot_one(cfl: float, data: dict[str, np.ndarray], out_dir: Path,
    ...
    values = np.concatenate((data["js"], data["model"]))
    ...
    ax.plot(data["x"], data["model"], "-o", color=color, lw=1.45,
    ...
    for ax, (title, xmin, xmax) in zip(axes.flat, WINDOWS):
        ...
        ax.plot(data["x"][local], data["model"][local], "-o", color=color,
...
def plot_sweep(results: dict[float, dict[str, np.ndarray]], out_dir: Path,
    ...
    for ax, cfl in zip(axes.flat, sorted(results)):
        ...
        ax.plot(data["x"], data["js"], "--", color="#777777", lw=1.15,
                label="WENO5-JS")
        ax.plot(data["x"], data["model"], "-o", color=color, lw=1.3,
    ...
    for cfl in sorted(results):
        data = results[cfl]
        ax.plot(data["x"], data["model"], lw=1.25, color=COLORS.get(cfl),
...
def run(args: argparse.Namespace) -> None:
    ...
    for cfl in args.cfls:
        ...
        model_result, model_meta = integrate_ssprk3(
            model, adv5.ssprk3, initial, cfl, args.t_end, device
        )
        js_row = metrics("WENO5-JS", cfl, x, js, exact, js_meta)
        model_row = metrics(
            model_label, cfl, x, model_result, exact, model_meta
        )
        rows.extend((js_row, model_row))
        results[cfl] = {
            "x": x,
            "initial": initial,
            "exact": exact,
        ...
        print("GSTE_DONE", model_row, flush=True)
