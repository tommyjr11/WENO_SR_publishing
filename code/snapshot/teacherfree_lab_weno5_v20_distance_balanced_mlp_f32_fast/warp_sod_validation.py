...
from warp_mlp_f32.run_weno5_circle_mlp_compare import (
    advance_one_step,
    density_field,
)
...
def _write_weno7_style_outputs(
    ...
    with (step_dir / "summary.txt").open("w", encoding="utf-8") as f:
        f.write(
            "validation_kind: trusted_weno5_sod_2d_warp_"
            "refsym_mlp_float32\n"
        )
...
def run_warp_sod_validation(
    ...
    return {
        "raw_step": raw_step,
        "step": accepted,
        "validation_kind": "trusted-weno5-sod-2d-warp-refsym-mlp-float32",
