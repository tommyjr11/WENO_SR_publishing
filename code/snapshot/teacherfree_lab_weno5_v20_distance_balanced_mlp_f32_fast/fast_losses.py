#!/usr/bin/env python3
"""Execution-optimized form of the WENO5 mixed-FP32 V20 loss.
...
SSPRK3 steps at a time. The numerical objective and dtypes are unchanged.
"""
...
def precompute_classical_trajectory(
    ...
    """Run the identical WENO5-JS/SSPRK3 baseline once and retain its states."""
    state = initial
    states = [state]
    for _ in range(n_steps):
        if step_operator is None:
            state = A.ssprk3_step_signed(
...
class TrajectoryStep(torch.nn.Module):
    """One complete SSPRK3 step and all original loss components."""
    ...
    def forward(
        ...
        reconstruction_loss = scaled_smooth_l1(
            G._reconstruct_all_heads(self.model, exact_now),
            exact_points,
            qscale,
        )
...
class ClassicalStep(torch.nn.Module):
    """Identical classical SSPRK3 step exposed as a compilable module."""
