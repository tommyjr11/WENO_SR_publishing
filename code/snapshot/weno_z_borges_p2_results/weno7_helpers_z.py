...
if wp is not None:
    ...
    def weno7_beta(q0: wp.float64, q1: wp.float64, q2: wp.float64, q3: wp.float64, q4: wp.float64, q5: wp.float64, q6: wp.float64) -> wp.vec4d:
        # This is algebraically identical to the expanded Jiang--Shu
        # quadratic forms (including their common factor 240), but it keeps
        # every beta non-negative in floating point.  The expanded form can
        # suffer catastrophic cancellation for constant characteristic
        # states with O(1e6) energy; an odd WENO-Z exponent would then turn a
        # tiny negative beta into a negative nonlinear weight.
        c0 = wp.float64(20.0) / wp.float64(3.0)
        c1 = wp.float64(260.0)
        c2 = wp.float64(781.0) / wp.float64(3.0)

        a00 = -wp.float64(2.0)*q0 + wp.float64(9.0)*q1 - wp.float64(18.0)*q2 + wp.float64(11.0)*q3
        a01 = -q0 + wp.float64(4.0)*q1 - wp.float64(5.0)*q2 + wp.float64(2.0)*q3
        a02 = -q0 + wp.float64(3.0)*q1 - wp.float64(3.0)*q2 + q3
        beta0 = c0*a00*a00 + c1*a01*a01 + c2*a02*a02

        a10 = q1 - wp.float64(6.0)*q2 + wp.float64(3.0)*q3 + wp.float64(2.0)*q4
        a11 = q2 - wp.float64(2.0)*q3 + q4
        a12 = -q1 + wp.float64(3.0)*q2 - wp.float64(3.0)*q3 + q4
        beta1 = c0*a10*a10 + c1*a11*a11 + c2*a12*a12

        a20 = -wp.float64(2.0)*q2 - wp.float64(3.0)*q3 + wp.float64(6.0)*q4 - q5
        a21 = q2 - wp.float64(2.0)*q3 + q4
        a22 = -q2 + wp.float64(3.0)*q3 - wp.float64(3.0)*q4 + q5
        beta2 = c0*a20*a20 + c1*a21*a21 + c2*a22*a22

        a30 = -wp.float64(11.0)*q3 + wp.float64(18.0)*q4 - wp.float64(9.0)*q5 + wp.float64(2.0)*q6
        a31 = wp.float64(2.0)*q3 - wp.float64(5.0)*q4 + wp.float64(4.0)*q5 - q6
        a32 = -q3 + wp.float64(3.0)*q4 - wp.float64(3.0)*q5 + q6
        beta3 = c0*a30*a30 + c1*a31*a31 + c2*a32*a32
    ...
    def weno7_lr_scalar(q0: wp.float64, q1: wp.float64, q2: wp.float64, q3: wp.float64, q4: wp.float64, q5: wp.float64, q6: wp.float64, lr: int, h: wp.float64, dorder: int) -> wp.float64:
        ...
        # Castro--Costa--Don Table 2 / Remark 10: the numerical experiments
        # use the optimal seventh-order global indicator, not the generic
        # even-r indicator from Theorem 2.  These beta values carry the
        # common factor 240 used by the C++ WENO7 implementation, hence
        # 240*h^3 is equivalent to the paper's epsilon=h^(r-1), r=4.
        tau7 = wp.abs(-beta[0] - wp.float64(3.0)*beta[1] + wp.float64(3.0)*beta[2] + beta[3])
        eps_z = wp.float64(240.0) * h * h * h
        ratio0 = tau7 / (beta[0] + eps_z)
        ratio1 = tau7 / (beta[1] + eps_z)
        ratio2 = tau7 / (beta[2] + eps_z)
        ratio3 = tau7 / (beta[3] + eps_z)
        a0 = d0 * (wp.float64(1.0) + ratio0 * ratio0 * ratio0)
        a1 = d1 * (wp.float64(1.0) + ratio1 * ratio1 * ratio1)
        a2 = d2 * (wp.float64(1.0) + ratio2 * ratio2 * ratio2)
        a3 = d3 * (wp.float64(1.0) + ratio3 * ratio3 * ratio3)
        asum = a0 + a1 + a2 + a3
    ...
    def weno7_gauss_lr_scalar(q0: wp.float64, q1: wp.float64, q2: wp.float64, q3: wp.float64, q4: wp.float64, q5: wp.float64, q6: wp.float64, lr: int, h: wp.float64, dorder: int) -> wp.float64:
        ...
        beta = weno7_beta(q0, q1, q2, q3, q4, q5, q6)
        # See weno7_lr_scalar: optimal tau_7 and paper-equivalent epsilon.
        tau7 = wp.abs(-beta[0] - wp.float64(3.0)*beta[1] + wp.float64(3.0)*beta[2] + beta[3])
        eps_z = wp.float64(240.0) * h * h * h
        ratio0 = tau7 / (beta[0] + eps_z)
        ratio1 = tau7 / (beta[1] + eps_z)
    ...
    @wp.func
    def weno7_normalized_weights(beta_raw: wp.vec4d, d: wp.vec4d, h: wp.float64, weight_kind: int) -> wp.vec4d:
        """Select one of the isolated JS-vs-Z audit conventions.

        0/1 use beta_raw/240 with epsilon=h^3 for JS/Z.  2/3 use the
        original beta_raw with epsilon=1e-40 for JS/Z.  4/5 use the
        original beta_raw with epsilon=h^3 and WENO-Z powers 1/2.
        """
        inv240 = wp.float64(1.0) / wp.float64(240.0)
        b0 = beta_raw[0]
        b1 = beta_raw[1]
        b2 = beta_raw[2]
        b3 = beta_raw[3]
        eps = wp.float64(1.0e-40)
        if weight_kind < 2:
            b0 = b0 * inv240
            b1 = b1 * inv240
            b2 = b2 * inv240
            b3 = b3 * inv240
            eps = h * h * h
        elif weight_kind == 4 or weight_kind == 5:
            eps = h * h * h
        a0 = wp.float64(0.0)
        a1 = wp.float64(0.0)
        a2 = wp.float64(0.0)
        a3 = wp.float64(0.0)
        if weight_kind == 0 or weight_kind == 2:
            r0 = wp.float64(1.0) / (b0 + eps)
            r1 = wp.float64(1.0) / (b1 + eps)
            r2 = wp.float64(1.0) / (b2 + eps)
            r3 = wp.float64(1.0) / (b3 + eps)
            a0 = d[0] * r0 * r0
            a1 = d[1] * r1 * r1
            a2 = d[2] * r2 * r2
            a3 = d[3] * r3 * r3
        else:
            tau = wp.abs(-b0 - wp.float64(3.0) * b1 + wp.float64(3.0) * b2 + b3)
            r0 = tau / (b0 + eps)
            r1 = tau / (b1 + eps)
            r2 = tau / (b2 + eps)
            r3 = tau / (b3 + eps)
            if weight_kind == 4:
                a0 = d[0] * (wp.float64(1.0) + r0)
                a1 = d[1] * (wp.float64(1.0) + r1)
                a2 = d[2] * (wp.float64(1.0) + r2)
                a3 = d[3] * (wp.float64(1.0) + r3)
            elif weight_kind == 5:
                a0 = d[0] * (wp.float64(1.0) + r0 * r0)
                a1 = d[1] * (wp.float64(1.0) + r1 * r1)
                a2 = d[2] * (wp.float64(1.0) + r2 * r2)
                a3 = d[3] * (wp.float64(1.0) + r3 * r3)
            else:
                a0 = d[0] * (wp.float64(1.0) + r0 * r0 * r0)
                a1 = d[1] * (wp.float64(1.0) + r1 * r1 * r1)
                a2 = d[2] * (wp.float64(1.0) + r2 * r2 * r2)
                a3 = d[3] * (wp.float64(1.0) + r3 * r3 * r3)
        asum = a0 + a1 + a2 + a3
        return wp.vec4d(a0 / asum, a1 / asum, a2 / asum, a3 / asum)


    @wp.func
    def weno7_lr_scalar_normalized(q0: wp.float64, q1: wp.float64, q2: wp.float64, q3: wp.float64, q4: wp.float64, q5: wp.float64, q6: wp.float64, lr: int, h: wp.float64, dorder: int, weight_kind: int) -> wp.float64:
        s = wp.vec4d()
        if lr == 1:
            s = wp.vec4d(
                stencil_weno7(q0, q1, q2, q3, 1, dorder),
                stencil_weno7(q1, q2, q3, q4, 2, dorder),
                stencil_weno7(q2, q3, q4, q5, 3, dorder),
                stencil_weno7(q3, q4, q5, q6, 4, dorder),
            )
        else:
            s = wp.vec4d(
                stencil_weno7(q0, q1, q2, q3, 2, dorder),
                stencil_weno7(q1, q2, q3, q4, 3, dorder),
                stencil_weno7(q2, q3, q4, q5, 4, dorder),
                stencil_weno7(q3, q4, q5, q6, 5, dorder),
            )
        d = weno7_optimal_weights(lr, 0)
        beta = weno7_beta(q0, q1, q2, q3, q4, q5, q6)
        omega = weno7_normalized_weights(beta, d, h, weight_kind)
        value = omega[0] * s[0] + omega[1] * s[1] + omega[2] * s[2] + omega[3] * s[3]
        return scale_derivative(value, h, dorder)


    @wp.func
    def weno7_gauss_lr_scalar_normalized(q0: wp.float64, q1: wp.float64, q2: wp.float64, q3: wp.float64, q4: wp.float64, q5: wp.float64, q6: wp.float64, lr: int, h: wp.float64, dorder: int, weight_kind: int) -> wp.float64:
        s = wp.vec4d()
        if lr == 1:
            s = wp.vec4d(
                stencil_weno7_2gauss(q0, q1, q2, q3, 1, dorder),
                stencil_weno7_2gauss(q1, q2, q3, q4, 2, dorder),
                stencil_weno7_2gauss(q2, q3, q4, q5, 3, dorder),
                stencil_weno7_2gauss(q3, q4, q5, q6, 4, dorder),
            )
        else:
            s = wp.vec4d(
                stencil_weno7_2gauss(q0, q1, q2, q3, 5, dorder),
                stencil_weno7_2gauss(q1, q2, q3, q4, 6, dorder),
                stencil_weno7_2gauss(q2, q3, q4, q5, 7, dorder),
                stencil_weno7_2gauss(q3, q4, q5, q6, 8, dorder),
            )
        d = weno7_optimal_weights(lr, 1)
        beta = weno7_beta(q0, q1, q2, q3, q4, q5, q6)
        omega = weno7_normalized_weights(beta, d, h, weight_kind)
        value = omega[0] * s[0] + omega[1] * s[1] + omega[2] * s[2] + omega[3] * s[3]
        return scale_derivative(value, h, dorder)
    ...
    @wp.func
    def weno7_lr_vec_conservative_normalized(q0: wp.vec4d, q1: wp.vec4d, q2: wp.vec4d, q3: wp.vec4d, q4: wp.vec4d, q5: wp.vec4d, q6: wp.vec4d, lr: int, h: wp.float64, dorder: int, weight_kind: int) -> wp.vec4d:
        return wp.vec4d(
            weno7_lr_scalar_normalized(q0[0], q1[0], q2[0], q3[0], q4[0], q5[0], q6[0], lr, h, dorder, weight_kind),
            weno7_lr_scalar_normalized(q0[1], q1[1], q2[1], q3[1], q4[1], q5[1], q6[1], lr, h, dorder, weight_kind),
            weno7_lr_scalar_normalized(q0[2], q1[2], q2[2], q3[2], q4[2], q5[2], q6[2], lr, h, dorder, weight_kind),
            weno7_lr_scalar_normalized(q0[3], q1[3], q2[3], q3[3], q4[3], q5[3], q6[3], lr, h, dorder, weight_kind),
        )


    @wp.func
    def weno7_gauss_vec_conservative_normalized(q0: wp.vec4d, q1: wp.vec4d, q2: wp.vec4d, q3: wp.vec4d, q4: wp.vec4d, q5: wp.vec4d, q6: wp.vec4d, lr: int, h: wp.float64, dorder: int, weight_kind: int) -> wp.vec4d:
        return wp.vec4d(
            weno7_gauss_lr_scalar_normalized(q0[0], q1[0], q2[0], q3[0], q4[0], q5[0], q6[0], lr, h, dorder, weight_kind),
            weno7_gauss_lr_scalar_normalized(q0[1], q1[1], q2[1], q3[1], q4[1], q5[1], q6[1], lr, h, dorder, weight_kind),
            weno7_gauss_lr_scalar_normalized(q0[2], q1[2], q2[2], q3[2], q4[2], q5[2], q6[2], lr, h, dorder, weight_kind),
            weno7_gauss_lr_scalar_normalized(q0[3], q1[3], q2[3], q3[3], q4[3], q5[3], q6[3], lr, h, dorder, weight_kind),
        )


    @wp.func
    def weno7_lr_vec_characteristic_normalized(q0: wp.vec4d, q1: wp.vec4d, q2: wp.vec4d, q3: wp.vec4d, q4: wp.vec4d, q5: wp.vec4d, q6: wp.vec4d, lr: int, h: wp.float64, dorder: int, direction: int, gamma: wp.float64, weight_kind: int) -> wp.vec4d:
        roe = roe_average_state(q3, q4, gamma)
        if lr == 2:
            roe = roe_average_state(q2, q3, gamma)
        c0 = con_to_char(q0, roe, direction, gamma)
        ...
        c6 = con_to_char(q6, roe, direction, gamma)
        cf = wp.vec4d(
            weno7_lr_scalar_normalized(c0[0], c1[0], c2[0], c3[0], c4[0], c5[0], c6[0], lr, h, dorder, weight_kind),
            weno7_lr_scalar_normalized(c0[1], c1[1], c2[1], c3[1], c4[1], c5[1], c6[1], lr, h, dorder, weight_kind),
            weno7_lr_scalar_normalized(c0[2], c1[2], c2[2], c3[2], c4[2], c5[2], c6[2], lr, h, dorder, weight_kind),
            weno7_lr_scalar_normalized(c0[3], c1[3], c2[3], c3[3], c4[3], c5[3], c6[3], lr, h, dorder, weight_kind),
        )
        return char_to_con(cf, roe, direction, gamma)
