...
if wp is not None:
    WENO5_USE_NUMERICAL_WEIGHTS = wp.constant(1)
    ...
    def weno5_lr_value(
        ...
        # Castro--Costa--Don time-dependent tests use epsilon=h^(r-1).
        # For WENO5, r=3 and the standard beta normalization gives h^2.
    ...
    def weno5_lr_value_characteristic(
        ...
        c_face = wp.vec4d(
            weno5_lr_value(c0[0], c1[0], c2[0], c3[0], c4[0], lr, h),
            weno5_lr_value(c0[1], c1[1], c2[1], c3[1], c4[1], lr, h),
            weno5_lr_value(c0[2], c1[2], c2[2], c3[2], c4[2], lr, h),
            weno5_lr_value(c0[3], c1[3], c2[3], c3[3], c4[3], lr, h),
        )
    ...
    def weno5_gauss_lr_value(
        ...
        # Same direction-wise paper epsilon as the face reconstruction.
        tau5 = wp.abs(beta0 - beta2)
        eps_z = h * h
        ratio0 = tau5 / (beta0 + eps_z)
        ratio1 = tau5 / (beta1 + eps_z)
    ...
    def compute_x_stage_weno_kernel(
        ...
        if j < ny + 6 and i < nx + 2:
            if characteristic != 0:
                ...
                ql = weno5_lr_value_characteristic(q0, q1, q2, q3, q4, 2, h, 1, gamma)
                qr = weno5_lr_value_characteristic(q0, q1, q2, q3, q4, 1, h, 1, gamma)
                for comp in range(4):
                ...
                for comp in range(4):
                    ...
                    q3s = u[j, i + 3, comp]
                    q4s = u[j, i + 4, comp]
                    temp_l[j, i, comp] = weno5_lr_value(q0s, q1s, q2s, q3s, q4s, 2, h)
                    temp_r[j, i, comp] = weno5_lr_value(q0s, q1s, q2s, q3s, q4s, 1, h)
    ...
    def compute_x_point_weno_kernel(
        ...
        if j < ny and i < nx + 2:
            for comp in range(4):
                ...
                point_l[j, i, comp] = weno5_gauss_lr_value(q0, q1, q2, q3, q4, loca, h)
                ...
                point_r[j, i, comp] = weno5_gauss_lr_value(q0, q1, q2, q3, q4, loca, h)
    ...
    def compute_y_stage_weno_kernel(
        ...
        if j < ny + 2 and i < nx + 6:
            if characteristic != 0:
                ...
                ql = weno5_lr_value_characteristic(q0, q1, q2, q3, q4, 2, h, 2, gamma)
                qr = weno5_lr_value_characteristic(q0, q1, q2, q3, q4, 1, h, 2, gamma)
                for comp in range(4):
                    temp_l[j, i, comp] = ql[comp]
                    temp_r[j, i, comp] = qr[comp]
            else:
