...
FILES_TO_COMPARE = [
    ...
    (
        "WENO5_Z_224",
        "weno_z_borges_p2_results/raw/shockbubble_3d/N224x88x88/"
        "weno5_z_p2/step_0840.bin",
        840,
    ),
    (
        "WENO5_JS_RK3_448",
...
STYLE_OVERRIDES = {
    ...
    "WENO5_Z_224": {
        "color": "#56B4E9", "linestyle": ":", "linewidth": 1.55,
        "marker": "o", "markersize": 2.7, "markevery": 12,
    ...
    "WENO7_Z_RK4_224": {
        "color": "#E69F00", "linestyle": ":", "linewidth": 1.55,
        "marker": "v", "markersize": 2.8, "markevery": 12,
        "markerfacecolor": "white", "markeredgewidth": 0.65, "zorder": 10,
    },
    "WENO7_JS_RK4_448": {
...
DISPLAY_LABEL_OVERRIDES = {
    ...
    "WENO5_Z_224": "WENO5-Z/RK3, 224 × 88²",
    ...
    "WENO7_Z_RK4_224": "WENO7-Z/RK4, 224 × 88²",
...
def resolve_input_files(source: Path, step: int):
    ...
    if step <= 0:
        candidates = sorted(source.glob("step_*.bin"))
        single_candidates = [
            path for path in candidates
            if re.fullmatch(r"step_\d+\.bin", path.name)
        ]
        if single_candidates:
            return "single", [single_candidates[-1]]
        raise FileNotFoundError(f"在 {source} 里找不到可用的单文件 step_*.bin")
...
def main():
    ...
    labels = [
        format_case_label(key) for key in legend_key_order
        if format_case_label(key) in handle_by_label
    ]
