...
if wp is not None:
    ...
    def compute_x_stage_point_kernel(
        ...
        if j < ny + 8 and i < nx + 2:
            ...
            if characteristic == 1:
                wh.write_vec(left, j, i, wh.weno7_lr_vec_characteristic_normalized(q0, q1, q2, q3, q4, q5, q6, 2, dx, 0, 1, gamma, weight_kind))
                wh.write_vec(right, j, i, wh.weno7_lr_vec_characteristic_normalized(q0, q1, q2, q3, q4, q5, q6, 1, dx, 0, 1, gamma, weight_kind))
            else:
                wh.write_vec(left, j, i, wh.weno7_lr_vec_conservative_normalized(q0, q1, q2, q3, q4, q5, q6, 2, dx, 0, weight_kind))
                wh.write_vec(right, j, i, wh.weno7_lr_vec_conservative_normalized(q0, q1, q2, q3, q4, q5, q6, 1, dx, 0, weight_kind))
    ...
    def compute_y_stage_point_kernel(
        ...
        if j < ny + 2 and i < nx + 8:
            ...
            if characteristic == 1:
                wh.write_vec(left, j, i, wh.weno7_lr_vec_characteristic_normalized(q0, q1, q2, q3, q4, q5, q6, 2, dy, 0, 2, gamma, weight_kind))
                wh.write_vec(right, j, i, wh.weno7_lr_vec_characteristic_normalized(q0, q1, q2, q3, q4, q5, q6, 1, dy, 0, 2, gamma, weight_kind))
            else:
                wh.write_vec(left, j, i, wh.weno7_lr_vec_conservative_normalized(q0, q1, q2, q3, q4, q5, q6, 2, dy, 0, weight_kind))
                wh.write_vec(right, j, i, wh.weno7_lr_vec_conservative_normalized(q0, q1, q2, q3, q4, q5, q6, 1, dy, 0, weight_kind))
    ...
    def compute_x_flux_point_kernel(
        ...
        if j < ny and i < nx + 1:
            ...
            state_l = wh.weno7_gauss_vec_conservative_normalized(ql0, ql1, ql2, ql3, ql4, ql5, ql6, loca, transverse_h, 0, weight_kind)
            state_r = wh.weno7_gauss_vec_conservative_normalized(qr0, qr1, qr2, qr3, qr4, qr5, qr6, loca, transverse_h, 0, weight_kind)
...
def compute_rhs(
    ...
    *,
    ...
    wp.launch(
        ...
        dim=(ny + 8, nx + 2),
        inputs=[src, arrays["x_l"], arrays["x_r"], nx, ny, wp.float64(params.dx), gamma, characteristic_i, weight_kind],
        device=device,
    )
    wp.launch(
        compute_x_flux_point_kernel,
        dim=(ny, nx + 1),
        inputs=[arrays["fx1"], arrays["x_l"], arrays["x_r"], wp.float64(dt / params.dx), wp.float64(params.dy), nx, ny, 1, gamma, solver_kind, reverse_i, weight_kind],
        device=device,
    )
    wp.launch(
        compute_x_flux_point_kernel,
        dim=(ny, nx + 1),
        inputs=[arrays["fx2"], arrays["x_l"], arrays["x_r"], wp.float64(dt / params.dx), wp.float64(params.dy), nx, ny, 2, gamma, solver_kind, reverse_i, weight_kind],
        device=device,
    )
    wp.launch(
        compute_y_stage_point_kernel,
        dim=(ny + 2, nx + 8),
        inputs=[src, arrays["y_l"], arrays["y_r"], nx, ny, wp.float64(params.dy), gamma, characteristic_i, weight_kind],
        device=device,
    )
    wp.launch(
        compute_y_flux_point_kernel,
        dim=(ny + 1, nx),
        inputs=[arrays["fy1"], arrays["y_l"], arrays["y_r"], wp.float64(dt / params.dy), wp.float64(params.dx), nx, ny, 1, gamma, solver_kind, reverse_i, weight_kind],
        device=device,
    )
    wp.launch(
        compute_y_flux_point_kernel,
        dim=(ny + 1, nx),
        inputs=[arrays["fy2"], arrays["y_l"], arrays["y_r"], wp.float64(dt / params.dy), wp.float64(params.dx), nx, ny, 2, gamma, solver_kind, reverse_i, weight_kind],
        device=device,
    )
    wp.launch(
...
def launch_shu_rk4_step(
    ...
    *,
    ...
    compute_rhs(arrays, params, "u", "rhs0", dt, device, solver_kind=solver_kind, characteristic=characteristic, reverse_upwind=False, boundary=boundary, weight_kind=weight_kind)
    wp.launch(shu_stage1_kernel, dim=(n0, n1), inputs=[arrays["u"], arrays["rhs0"], arrays["u1"], n0, n1, wp.float64(dt)], device=device)

    compute_rhs(arrays, params, "u", "rhs_t0", dt, device, solver_kind=solver_kind, characteristic=characteristic, reverse_upwind=True, boundary=boundary, weight_kind=weight_kind)
    compute_rhs(arrays, params, "u1", "rhs1", dt, device, solver_kind=solver_kind, characteristic=characteristic, reverse_upwind=False, boundary=boundary, weight_kind=weight_kind)
    ...
    compute_rhs(arrays, params, "u1", "rhs_t1", dt, device, solver_kind=solver_kind, characteristic=characteristic, reverse_upwind=True, boundary=boundary, weight_kind=weight_kind)
    compute_rhs(arrays, params, "u2", "rhs2", dt, device, solver_kind=solver_kind, characteristic=characteristic, reverse_upwind=False, boundary=boundary, weight_kind=weight_kind)
    ...
    compute_rhs(arrays, params, "u3", "rhs3", dt, device, solver_kind=solver_kind, characteristic=characteristic, reverse_upwind=False, boundary=boundary, weight_kind=weight_kind)
...
def run_from_initial(
    ...
    *,
    ...
    while t < params.t_end and step < max_steps:
        ...
        if dt <= 0.0:
            break
        launch_shu_rk4_step(arrays, params, dt, device, riemann_solver=riemann_solver, characteristic=characteristic, boundary=boundary, weight_kind=weight_kind)
        t += dt
        step += 1
    ...
    summary: dict[str, object] = {
        "weight_kind": "js" if weight_kind in (0, 2) else "z",
        "weno_z_power": 1 if weight_kind == 4 else (2 if weight_kind == 5 else (3 if weight_kind in (1, 3) else 0)),
        "beta_convention": "beta_raw/240" if weight_kind < 2 else "beta_raw",
        "epsilon_convention": "h^3" if weight_kind < 2 or weight_kind in (4, 5) else "1e-40",
        "steps": int(step),
        "t": float(t),
