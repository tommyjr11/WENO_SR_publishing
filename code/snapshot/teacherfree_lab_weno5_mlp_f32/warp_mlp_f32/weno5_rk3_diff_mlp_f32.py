#!/usr/bin/env python3
"""Differentiable one-step WENO5/RK3 scaffold.

This file intentionally does not replace weno5_rk3_warp.py.  It builds a
separate one-step path with fixed dt, selectable ghost cells, FORCE fluxes, and
non-in-place RK stage arrays so Warp Tape can record a clean U^n -> U^{n+1}
graph.
"""
...
if wp is not None:
    ...
    MLP_HIDDEN2 = 6
    MLP_HIDDEN3 = 6
    Vec5f = wp.types.vector(length=MLP_INPUTS, dtype=wp.float32)
    VecH1f = wp.types.vector(length=MLP_HIDDEN1, dtype=wp.float32)
    VecH2f = wp.types.vector(length=MLP_HIDDEN2, dtype=wp.float32)
    VecH3f = wp.types.vector(length=MLP_HIDDEN3, dtype=wp.float32)
    ...
    @wp.func
    def swish(x: wp.float32) -> wp.float32:
        return x / (wp.float32(1.0) + wp.exp(-x))
    ...
    @wp.func
    def weno5_nn_features(q0: wp.float64, q1: wp.float64, q2: wp.float64, q3: wp.float64, q4: wp.float64) -> Vec5f:
        ...
        features = Vec5f()
        features[0] = wp.float32(delta0 * inv_delta_max)
        features[1] = wp.float32(delta1 * inv_delta_max)
        features[2] = wp.float32(delta2 * inv_delta_max)
        features[3] = wp.float32(gamma_s)
        features[4] = wp.float32(scale_feature)
        return features
    ...
    def mlp_weights(
        ...
        inv_sum = wp.float32(1.0) / (e0 + e1 + e2)
        beta_ratio0 = e0 * inv_sum
        beta_ratio1 = e1 * inv_sum
        beta_ratio2 = e2 * inv_sum

        eps_ratio = wp.float64(1.0e-12)
        badness_ratio_scale = wp.float64(3.0)
        scaled_beta0 = badness_ratio_scale * wp.float64(beta_ratio0)
        scaled_beta1 = badness_ratio_scale * wp.float64(beta_ratio1)
        scaled_beta2 = badness_ratio_scale * wp.float64(beta_ratio2)
    ...
    def smooth_weight_penalty(q0: wp.float64, q1: wp.float64, q2: wp.float64, q3: wp.float64, q4: wp.float64, w: wp.vec3d, head: int) -> wp.float64:
        ...
        smooth = wp.float64(1.0) - wp.float64(features[3])
        c = optimal_weights(head)
        dw0 = w[0] - c[0]
        dw1 = w[1] - c[1]
        dw2 = w[2] - c[2]
...
def make_mlp_params(device: str, seed: int = 1) -> dict[str, object]:
    ...
    w1 = rng.normal(0.0, np.sqrt(1.0 / float(inputs)), size=(1, inputs, hidden1)).astype(np.float32)
    b1 = np.zeros((1, hidden1), dtype=np.float32)
    w2 = rng.normal(0.0, np.sqrt(1.0 / float(hidden1)), size=(1, hidden1, hidden2)).astype(np.float32)
    b2 = np.zeros((1, hidden2), dtype=np.float32)
    w3 = rng.normal(0.0, np.sqrt(1.0 / float(hidden2)), size=(1, hidden2, hidden3)).astype(np.float32)
    b3 = np.zeros((1, hidden3), dtype=np.float32)
    w4 = np.zeros((1, hidden3, 3), dtype=np.float32)
    r_init = np.array([1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0], dtype=np.float32)
    init_logits = np.log(r_init)
    init_logits = init_logits - np.mean(init_logits)
    b4 = (6.0 * np.arctanh(init_logits / 6.0))[None, :].astype(np.float32)
    ...
    return {
        ...
        "w4": wp.array(w4, dtype=wp.float32, device=device, requires_grad=True),
        "b4": wp.array(b4, dtype=wp.float32, device=device, requires_grad=True),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weno-space", choices=("conserved", "characteristic"), default="characteristic")
