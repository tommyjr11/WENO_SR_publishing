...
from teacherfree_lab_weno5_mlp_f32 import weno5_core as W
from teacherfree_lab_weno5_mlp_f32.apost_advect import IC_DEFAULT_PROBS, IC_KINDS, make_ic, rollout_loss
from teacherfree_lab_weno5_mlp_f32 import warp_sod_validation as SV
...
def bound_violation(omega_fn, q: torch.Tensor, tol: float = 1.0e-3) -> torch.Tensor:
    ...
    rng = torch.clamp(hi - lo, min=1.0e-12)
...
def make_monotone_stencils(n: int, device: torch.device, gen: torch.Generator) -> torch.Tensor:
    ...
    inc = inc / torch.clamp(torch.sum(inc, dim=1, keepdim=True), min=1.0e-12)
...
def make_ampgate_stencils(
    ...
    rng = torch.clamp(torch.max(q, dim=1).values - torch.min(q, dim=1).values, min=1.0e-12)
...
def run(args: argparse.Namespace) -> None:
    ...
    for h in horizons:
        assert abs(h * args.cfl - round(h * args.cfl)) < 1.0e-6, (
    ...
    for name in IC_KINDS:
        ...
        probes[name] = (ic, max(float(cls), 1.0e-30))
    ...
    print(
        ...
        f"ic_probs={ic_probs} precision=mlp_float32_state_float64 sod_eval={args.sod_eval} "
    ...
    for step in range(1, args.steps + 1):
        ...
        if step == 1 or step % args.checkpoint_interval == 0 or step == args.steps:
            ...
            W.save_checkpoint(ckpt, model, {"raw_step": step, "recipe": "teacherfree_weno5_apost_advect_mlp_f32_state_f64"})
        ...
        if step == 1 or step % args.eval_interval == 0 or step == args.steps:
            ...
            if args.sod_eval and args.sod_nx > 0 and args.sod_ny > 0:
                ...
                payload = W.checkpoint_payload(
                    ...
                    {
                        "raw_step": step,
                        "recipe": "teacherfree_weno5_apost_advect_mlp_f32_state_f64",
                    },
                )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ampgate-amp-max", type=float, default=3.0e-3)
    ...
    p.add_argument("--sod-ny", type=int, default=8)
