#!/usr/bin/env python3
"""Trusted WENO5/RK3 path with reflection-symmetric FP32 MLP inference.

State, flux, characteristic projection, Riemann fluxes, and SSPRK3 remain
FP64. Only the five MLP inputs after feature construction, MLP parameters,
hidden activations, logits, and softmax are FP32. Ratios are cast back to FP64
...
if wp is not None:
    ...
    def mlp_badness_ratios(
        ...
        return wp.vec3f(e0 * inv_sum, e1 * inv_sum, e2 * inv_sum)
    ...
    def mlp_weights(
        ...
        beta_ratio0 = wp.float64(0.5) * (
            wp.float64(direct[0]) + wp.float64(reflected_raw[2])
        )
        beta_ratio1 = wp.float64(0.5) * (
            wp.float64(direct[1]) + wp.float64(reflected_raw[1])
        )
        beta_ratio2 = wp.float64(0.5) * (
            wp.float64(direct[2]) + wp.float64(reflected_raw[0])
        )
