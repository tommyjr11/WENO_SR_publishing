#!/usr/bin/env bash
# V20 controlled precision ablation: FP32 MLP, otherwise the FP64 V20 recipe.
set -euo pipefail
...
GPU_ID="${GPU_ID:-0}"
DETACH="${DETACH:-1}"
OUT="${OUT:-teacherfree_lab_weno5_v20_distance_balanced_mlp_f32/runs/apost_weno5_v20_distance_balanced_mlp_f32_cfl05_200k}"
...
COMMAND=(
  "$PYTHON_BIN" -u \
  -m teacherfree_lab_weno5_v20_distance_balanced_mlp_f32.train_weno5_v20_mlp_f32
...
launch_monitor () {
  ...
  nohup setsid env CUDA_VISIBLE_DEVICES="$GPU_ID" "$PYTHON_BIN" -u \
    -m teacherfree_lab_weno5_v20_distance_balanced_mlp_f32.sod_checkpoint_monitor_mlp_f32 \
  ...
  echo "launched mixed-FP32 symmetric Warp Sod monitor pid=$!"
}
...
if [[ "$DETACH" == "1" ]]; then
  ...
  echo "launched WENO5 V20 MLP-FP32 distance-balanced training on GPU $GPU_ID pid=$PID"
  launch_monitor
else
  echo "$$" > "$OUT/pid.txt"
