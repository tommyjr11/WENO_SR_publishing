#!/usr/bin/env python3
"""Trusted 2D Warp/RK3 Sod validation using mixed-precision inference.
...
from teacherfree_lab_weno5_v20_distance_balanced_mlp_f32.warp_mlp_f32.run_weno5_circle_mlp_compare_v20_mlp_f32 import (
    advance_one_step,
    density_field,
)
...
def _write_weno7_style_outputs(
    ...
    with (step_dir / "summary.txt").open("w", encoding="utf-8") as f:
        f.write("validation_kind: weno7_style_sod_2d_warp_refsym_mlp_float32\n")
        f.write("mlp_forward: 0.5*(M(x)+P*M(Px))\n")
        f.write("precision: mlp_float32_state_float64\n")
...
def run_warp_sod_validation(
    ...
    return {
        "raw_step": raw_step,
        "step": accepted,
        "validation_kind": "weno7-style-sod-2d-warp-refsym-mlp-float32",
        "mlp_forward": "0.5*(M(x)+P*M(Px))",
