#!/usr/bin/env bash
# Restartable five-method Titarev--Toro suite at CFL=0.8 on a 2000x10 strip.
set -uo pipefail
...
FIGURE_DIR="$ROOT/for_paper_results/figures/titarev_toro_cfl08_N${NX}"
TABLE_DIR="$ROOT/for_paper_results/tables/titarev_toro_cfl08_N${NX}"
LOG_DIR="$ROOT/for_paper_results/logs"
STATUS_FILE="$LOG_DIR/titarev_toro_cfl08_n${NX}_ny${NY}.status"
...
is_complete() {
  ...
  [[ -f "$npz" && -f "$metadata" ]] \
    ...
    && grep -q '"nx": 2000' "$metadata" \
    && grep -q '"ny": 10' "$metadata"
}

refresh_figures() {
  run_phase \
    "refresh Titarev--Toro ${NX}x${NY} CFL=0.8 figures" \
