...
def launch_weno7_ader4_step(
    ...
    if riemann_solver not in ("evilin", "hllc"):
        raise ValueError("weno7_ader4_warp_classical_only.py supports --riemann-solver evilin or hllc")
    if mlp_params is not None:
        raise ValueError("weno7_ader4_warp_classical_only.py is classical-only; use warp_weno7_mlp_beta_provider.py for MLP.")
    nx = params.nx
    ny = params.ny
    gc = params.ghost
    nx_total = nx + 2 * gc
