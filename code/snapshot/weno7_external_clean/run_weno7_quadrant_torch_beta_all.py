...
import warp_weno7_external_overwrite_fullall as ext
...
class TorchWeno7Beta:
    ...
    def _compute_beta_conservative(self, q: list[torch.Tensor]) -> torch.Tensor:
        shape = q[0].shape[:-1]
        beta = torch.empty(shape + (32,), dtype=torch.float64, device=self.device)
        for side, lr in ((0, 1), (1, 2)):
            for comp in range(4):
                vals = torch.stack([qq[..., comp] for qq in q], dim=-1)
                if lr == 1:
                    b = self._mlp_beta(vals)
                else:
                    b = torch.flip(self._mlp_beta(torch.flip(vals, dims=(-1,))), dims=(-1,))
    ...
    @torch.no_grad()
    def fill_normal_x(self, arrays: dict[str, object], params: wh.Params) -> None:
    ...
    @torch.no_grad()
    def fill_normal_y(self, arrays: dict[str, object], params: wh.Params) -> None:
        u = wp.to_torch(arrays["u"])
    ...
    @torch.no_grad()
    def fill_cross_x(self, arrays: dict[str, object], params: wh.Params) -> None:
        out = wp.to_torch(arrays["beta_cross_x"])
        for group, names in enumerate((("l0", "l1", "l2", "l3"), ("r0", "r1", "r2", "r3"))):
            for base, name in enumerate(names):
                a = wp.to_torch(arrays[name])
                q = [a[k + 1 : k + 1 + params.ny, : params.nx + 2, :] for k in range(7)]
                out[..., group * 128 + base * 32 : group * 128 + base * 32 + 32].copy_(self._compute_beta_conservative(q))
        self._sync_after_copy()

    @torch.no_grad()
    def fill_cross_y(self, arrays: dict[str, object], params: wh.Params) -> None:
...
def launch_weno7_ader4_step_torch_beta_all(
    ...
    boundary: str,
    riemann_solver: str,
    beta_model: TorchWeno7Beta,
    eno_cutoff: bool,
) -> None:
    if riemann_solver not in ("evilin", "hllc"):
        raise ValueError("torch-beta all runner supports evilin or hllc")
    nx = params.nx
    ny = params.ny
    gc = params.ghost
    nx_total = nx + 2 * gc
    ...
    beta_model.fill_normal_x(arrays, params)
    wp.launch(
        ext.overwrite_x_normal_mlp_all_kernel,
        dim=(ny + 8, nx + 2),
        inputs=[arrays["u"], arrays["beta_x"], arrays["l0"], arrays["r0"], arrays["l1"], arrays["r1"], arrays["l2"], arrays["r2"], arrays["l3"], arrays["r3"], nx, ny, wp.float64(params.dx), wp.float64(params.gamma), eno_cutoff_i],
        device=device,
    )
    wp.synchronize()
    beta_model.fill_cross_x(arrays, params)
    for loca in (1, 2):
        for tl_name, tr_name, stage_loca in (("tl1", "tr1", 1), ("tl2", "tr2", 2), ("tl3", "tr3", 3)):
            wp.launch(
                ext.compute_x_cross_stage_ader4_external_beta_slot_kernel,
                dim=(ny, nx + 2),
                inputs=[arrays[tl_name], arrays[tr_name], arrays["l0"], arrays["r0"], arrays["l1"], arrays["r1"], arrays["l2"], arrays["r2"], arrays["l3"], arrays["r3"], arrays["beta_cross_x"], nx, ny, wp.float64(params.dy), wp.float64(dt), loca, stage_loca, wp.float64(params.gamma), eno_cutoff_i],
                device=device,
            )
        wp.launch(
    ...
    beta_model.fill_normal_y(arrays, params)
    wp.launch(
        ext.overwrite_y_normal_mlp_all_kernel,
        dim=(ny + 2, nx + 8),
        inputs=[arrays["u"], arrays["beta_y"], arrays["l0"], arrays["r0"], arrays["l1"], arrays["r1"], arrays["l2"], arrays["r2"], arrays["l3"], arrays["r3"], nx, ny, wp.float64(params.dy), wp.float64(params.gamma), eno_cutoff_i],
        device=device,
    )
    wp.synchronize()
    beta_model.fill_cross_y(arrays, params)
    for loca in (1, 2):
        for tl_name, tr_name, stage_loca in (("tl1", "tr1", 1), ("tl2", "tr2", 2), ("tl3", "tr3", 3)):
            wp.launch(
                ext.compute_y_cross_stage_ader4_external_beta_slot_kernel,
                dim=(ny + 2, nx),
                inputs=[arrays[tl_name], arrays[tr_name], arrays["l0"], arrays["r0"], arrays["l1"], arrays["r1"], arrays["l2"], arrays["r2"], arrays["l3"], arrays["r3"], arrays["beta_cross_y"], nx, ny, wp.float64(params.dx), wp.float64(dt), loca, stage_loca, wp.float64(params.gamma), eno_cutoff_i],
                device=device,
            )
        wp.launch(
...
def run_solution(
    ...
    while t < args.t_end and step < args.max_steps:
        ...
        if mlp_params is None:
            ...
            launch_weno7_ader4_step_torch_beta_all(
                arrays,
                params,
                dt,
                args.device,
...
def run(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        "external_beta_all=torch-beta-overwrite",
        flush=True,
    )
    ...
    write_summary(
        args.out_dir / "summary.txt",
        {
            ...
            "external_beta_all": bool(args.run_mlp),
        },
        summaries,
    )
